    ...
    public static class LeagueTableHelper
    {
        ...
        public static List<LeagueTable> FetchLeagueTablesByUserId(int eventId, string userId)
        {
            ...
            foreach (var fixturePrediction in fixturePredictions)
            {
                ...
                if (eventTeamHome.League == eventTeamAway.League)
                {
                    ...
                    leagueTableTeamHome.Played += 1;
                    leagueTableTeamHome.GoalsFor += fixturePrediction.HomePrediction ?? default(short);
                    leagueTableTeamHome.GoalsAgainst += fixturePrediction.AwayPrediction ?? default(short);

                    leagueTableTeamAway.Played += 1;
                    leagueTableTeamAway.GoalsFor += fixturePrediction.AwayPrediction ?? default(short);
                    leagueTableTeamAway.GoalsAgainst += fixturePrediction.HomePrediction ?? default(short);

                    if (fixturePrediction.HomePrediction > fixturePrediction.AwayPrediction)
                    {
                        //Home Win
                        leagueTableTeamHome.Won += 1;
                        leagueTableTeamAway.Lost += 1;
                        leagueTableTeamHome.Points += 3;
                    }
                    else if (fixturePrediction.HomePrediction < fixturePrediction.AwayPrediction)
                    {
                        //Away Win
                        leagueTableTeamAway.Won += 1;
                        leagueTableTeamHome.Lost += 1;
                        leagueTableTeamAway.Points += 3;

                    }
                    else if (fixturePrediction.HomePrediction == fixturePrediction.AwayPrediction)
                    {
                        //Draw
                        leagueTableTeamHome.Draws += 1;
                        leagueTableTeamAway.Draws += 1;
                        leagueTableTeamHome.Points += 1;
                        leagueTableTeamAway.Points += 1;
                    }
                }
            }
